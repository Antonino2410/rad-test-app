...
if page == "Carica Stock In Mano":
    ...
    if uploaded_file:
        ...
        if COL_ITEM_CODE in df.columns and COL_QUANTITA in df.columns:
            for _, row in df.iterrows():
                item = str(row[COL_ITEM_CODE]).strip()
    ...
    if richiesta.empty:
        ...
        for item, qta_richiesta in richieste_aggregate.items():
            qta_in_mano = stock_in_mano.get(str(item).strip(), {}).get("quantità", 0)
            loc = stock_in_mano.get(str(item).strip(), {}).get("location", "non definita")
            ...
            if st.button("Verifica disponibilità per ordine"):
                ...
                for _, riga in grouped.iterrows():
                    ...
                    if qta_stock >= richiesta_qta:
                        rows.append({"Item Code": item, "Richiesta": richiesta_qta, "Disponibile": qta_stock, "Location": loc_stock, "Prelievo": "Stock In Mano"})
                    else:
                        ...
                        if suggerimenti:
                            rows.append({"Item Code": item, "Richiesta": richiesta_qta, "Disponibile": qta_stock, "Location": loc_stock, "Prelievo": "; ".join(suggerimenti)})
                        else:
                            rows.append({"Item Code": item, "Richiesta": richiesta_qta, "Disponibile": qta_stock, "Location": loc_stock, "Prelievo": "Non disponibile"})

                if rows:
                    df_result = pd.DataFrame(rows)
                    df_result["Item Code"] = df_result["Item Code"].astype(str)
                    df_result = df_result.sort_values("Item Code").reset_index(drop=True)
